...
/* GC9101A vendor register initialization sequence. */
#define GC9101A_INIT_CMD_MAX_DATA 12U

static const struct {
	uint8_t cmd;
	uint8_t len;
	uint8_t data[GC9101A_INIT_CMD_MAX_DATA];
} gc9101a_default_init[] = {
	{0xEBU, 1U, {0x14U}},
	{0x84U, 1U, {0x40U}},
	{0x85U, 1U, {0xFFU}},
	{0x86U, 1U, {0xFFU}},
...
int gc9101a_regs_init(const struct device *dev)
{
	...
	const struct gc9101a_regs *regs = config->regs;
	int ret;

	for (int i = 0; i < ARRAY_SIZE(gc9101a_default_init); i++) {
		ret = gc9x01x_transmit(dev, gc9101a_default_init[i].cmd,
				       gc9101a_default_init[i].data,
				       gc9101a_default_init[i].len);
		if (ret < 0) {
			return ret;
		}
	}
